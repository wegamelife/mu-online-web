...
export async function resetLife(username, characterName) {
  ...
  const query1 = `
        ...
        SET cLevel=('1'),
            ...
            RestTime = GETDATE(),
            LevelUpPoint = (ResetLife + 1) * 600 + 1000,
            Strength = 30,
            Dexterity = 30,
            Vitality = 30,
            Energy = 30
...
export async function autoAddPoints(username, characterName) {
  const conn = await getConnection();

  const character = await getCharacter(username, characterName);
  const sumPoints =
    character["ResetLife"] * 600 + 1000 + character["cLevel"] * 50;
  const eachPoints = parseInt(sumPoints / 4);

  const query1 = `update Muonline.dbo.Character
                    set Strength     = ${eachPoints},
                        Dexterity    = ${eachPoints},
                        Vitality     = ${eachPoints},
                        Energy       = ${eachPoints},
                        LevelUpPoint = 0
                    where AccountID = ?
