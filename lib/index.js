...
export async function updateItemsSockets(username, items, itemName) {
  const query1 = `update warehouse
                    set Items = Muonline.dbo.fn_hexstrtovarbin('${items}')
                    where AccountID = ?`;
 ...
 * @return {Promise<unknown>}
 */
export async function recycle(username, items, itemInfo) {
  const query1 = `update warehouse
                    set Items = Muonline.dbo.fn_hexstrtovarbin('${items}')
                    where AccountID = ?`;
  ...
  let { name, price } = itemInfo;
  // 限制超过5000积分
  if (price > 5000) {
    price = 5000;
  }

  await updateUserJF(username, price, "add");
  logMe(`回收 ${username} ${name} 获得积分 ${price}`);
  return await executeSql(query1, username);
}
