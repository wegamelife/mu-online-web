    ...
    const query =
        `SELECT *
         FROM MuOnline.dbo.Character as c
                  left join Muonline.dbo.T_MasterLevelSystem as m on c.Name = m.CHAR_NAME collate Chinese_PRC_CI_AI_WS
         order by ResetLife DESC, MASTER_LEVEL DESC, cLevel DESC, RestTime ASC`;
    const conn = await getConnection();
...
export async function resetLife(username, characterName) {
    ...
    const query1 = `
        ...
        SET cLevel=('10'),
            ...
            RestTime = GETDATE(),
            LevelUpPoint = (ResetLife + 1) * ${RESET_LIFE_PER_POINTS} + 1000 + 10 * ${LEVEL_UP_POINTS},
            Strength = 30,
            Dexterity = 30,
            Vitality = 30,
            Energy = 30
...
export async function addPoints(
    ...
    const LevelUpPoint =
        totalPoints - _Strength - _Dexterity - _Vitality - _Energy;
...
export async function clearPoints(username, characterName) {
    ...
    const query1 = `update Muonline.dbo.Character
                    set Strength     = 30,
                        ...
                        Vitality     = 30,
                        Energy       = 30,
                        LevelUpPoint = (1000 + ResetLife * ${RESET_LIFE_PER_POINTS} + cLevel * ${LEVEL_UP_POINTS})
                    where AccountID = ?
                      and Name = ?`;
