...
export async function createUser(username, password) {
  ...
  const query1 = `insert into MuOnline.dbo.MEMB_INFO
                    (memb___id, memb__pwd, memb_name, appl_days, modi_days, out__days, true_days, ctl1_code, bloc_code,
                     sno__numb, jf)
...
export async function getCharactersByAccount(username) {
  const conn = await getConnection();
  const query1 = `select * from Muonline.dbo.Character where AccountID = ?`;
