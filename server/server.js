...
async function getByUserName(userName) {
  const query1 = `select * from MuOnline.dbo.MEMB_INFO where memb___id = ?`;
  const conn = await getConnection();
  return new Promise((resolve, reject) => {
...
async function createUser(username, password) {
  const query1 = `insert into MuOnline.dbo.MEMB_INFO (memb___id, memb__pwd, memb_name, sno__numb, appl_days, modi_days, out__days, true_days,mail_chek, bloc_code,ctl1_code, jf, partation, servercode, usedtime) values (?, ?, 'name', '1111111111111', GETDATE(), GETDATE(), GETDATE(), GETDATE(), 0, 0, 0, 0, 0, 0, 0)`;
